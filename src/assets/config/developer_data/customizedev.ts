const developerData = {
  developer_username: "Nishant Banjade",
  navbar_home: "home",
  navbar_about: "about",
  navbar_contact: "contact",
  navbar_terminal: "terminal",
  // Make sure to make it short and crisp
  herobody_img_description: "Desktop computer vector image illustration",
  developer_quote:
    "Behind every successful program lies the dedication of a developer who, like an alchemist, transforms ideas into lines of code, turning dreams into reality.",
    
  // customize the typeWriter speed
  typeWriter_speed: 22,

  // About section / Introduction
  about_title_name: "Introduction",
  about_description: "Hi , I'm Nishant Banjade, a Software Engineer at Ellucian, dedicated to improving education through technology. My experience includes developing a CRM system, Plugins, APIs & Frontend logics for Universities around the world. I'm actively learning and working on full-stack projects using React, TypeScript, and .NET, and I'm passionate about open-source contributions."
};
export default developerData;
